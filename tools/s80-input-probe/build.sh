#!/bin/bash
# Build the ARM input contract probe (Symbian OS 7.0s, EKA1).
...
OUT=${OUT:-$SRC/build}
NAME=inputprobe
UID3=0x0A5C7E5B     # test range (0x01000000-0x0FFFFFFF)
...
"${LD[@]}" "$OUT/$NAME.exp" -Map "$OUT/$NAME.map" -o "$OUT/$NAME.pe" "$LIB/EEXE.LIB" \
  --whole-archive "$OUT/$NAME.in" --no-whole-archive "${LIBS[@]}"
petran "$OUT/$NAME.pe" "$OUT/InputProbe.exe" -nocall -uid1 0x1000007a -uid2 0x00000000 -uid3 $UID3 \
  -heap 0x1000 0x100000 -stack 0x2000
ls -l "$OUT/InputProbe.exe"; od -An -tx4 -N16 "$OUT/InputProbe.exe"
