#!/bin/bash
# build.sh -- build BootContracts.exe (Symbian OS 7.0s, EKA1 ARMI EXE) on Linux; recipe as tools/s80-socktest.
...
LIBS=("$LIB/EGCC.LIB" "$LIB/EUSER.lib" "$LIB/EFSRV.lib" "$LIB/HAL.lib")
...
"${LD[@]}" "$OUT/$NAME.exp" -Map "$OUT/$NAME.map" -o "$OUT/$NAME.pe" "$LIB/EEXE.LIB" \
  --whole-archive "$OUT/$NAME.in" --no-whole-archive "${LIBS[@]}"
petran "$OUT/$NAME.pe" "$OUT/BootContracts.exe" -nocall -uid1 0x1000007a -uid2 0x00000000 -uid3 $UID3 \
  -heap 0x1000 0x10000 -stack 0x2000
ls -l "$OUT/BootContracts.exe"; od -An -tx4 -N16 "$OUT/BootContracts.exe"
